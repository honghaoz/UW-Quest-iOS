...
#define PROFILES_DEFAULT_HOST @"api.localytics.com"
#define PROFILES_PATH @"/profile/v1/apps/%@/profiles/%@"
...
- (NSString *)blobHeaderStringWithSequenceNumber:(int)nextSequenceNumber
{
    ...
    if (![[self db] isPushTokenNull])
    {
        ...
        if (types != UIRemoteNotificationTypeNone) pushToken = [[self db] pushToken];
    ...
    if (![[self db] isDevPushTokenNull])
    {
        ...
        if (types != UIRemoteNotificationTypeNone) devPushToken = [[self db] devPushToken];
...
- (NSString *)customerIdentifier
{
    ...
    [customerIdentfer appendString:
     [self formatAttributeWithName:[self escapeString:@"cid"]
                             value:[self escapeString:customerIdTuple.customerId]
                             first:NO]];
    // add user type
...
- (void)setProfileValue:(NSObject<NSCopying> *)value forAttribute:(NSString *)attribute
{
	dispatch_async(_queue, ^{
		@try {
            ...
            if ([self isValidValueType:localValue]){
                CustomerIdTuple *customerIdTuple = [self customerIdTuple];
                NSDictionary *profileDictionary = @{@"app_id": self.applicationKey,
                                                    @"cid": customerIdTuple.customerId,
                                                    @"attributes": @{attribute: localValue}};
                NSString *jsonString = [self toJSON:profileDictionary];
...
- (void)uploadProfileEvents{
	dispatch_group_async(_criticalGroup, _queue, ^{
		@try {
            ...
            // loop until we either run out of profile events in the DB or we fail to upload a db
            do{
                // try to read the next profile event
                ProfileEventTuple *profileEventTuple = [[self.db getNextProfileEventTuple]retain];
                if (profileEventTuple){
                    // try to upload the profile event
                    success = [self uploadProfileEventTuple:profileEventTuple];
                    if (success){
                        // if we succed then remove the top profile event
                        [self.db deleteProfileEventTuple:profileEventTuple];
                    } else {
                        // if we fail to upload then we're done trying to upload profile events
                        done = YES;
                    }
                } else {
                    // if we don't read a top profile event that means there aren't any more so we're done
                    done = YES;
                }
                [profileEventTuple release];
            }while(!done);
...
- (BOOL)uploadProfileEventTuple:(ProfileEventTuple *)profileEventTuple{
    BOOL success = NO;
    ...
    NSString *customerId = jsonDictionary[@"cid"];
    // if we have a problem getting the customerId out of the json ...
    if (!customerId){
        // ... get the current custoemrId
        customerId = self.customerIdTuple.customerId;
    }
...
- (NSString *)toJSON:(NSObject *)obj{
    ...
    if ((obj == nil) || (obj == [NSNull null])){
        ...
        for (NSObject *element in array){
            ...
            [jsonString appendString:[self toJSON:element]];
            first = NO;
        }
        [jsonString appendString:@"]"];
        ...
        for (NSString *key in keys){
            ...
            [jsonString appendString:[self toJSON:key]];
            [jsonString appendString:@":"];
            NSObject *value = [dictionary valueForKey:key];
            [jsonString appendString:[self toJSON:value]];
            first = NO;
        }
        [jsonString appendString:@"}"];
    } else if ([obj isKindOfClass:[NSString class]]){
        NSString *string = (NSString *)obj;
        [jsonString appendFormat:@"\"%@\"",[self escapeString:string]];
