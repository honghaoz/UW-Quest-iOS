...
+ (NSString *)advertisingIdentifier
{
	...
	if ([LocalyticsUtil advertisingIdentifierEnabled])
	{
		...
		if (advertisingClass) {
			...
			adId = (NSString *)[[[advertisingClass performSelector:NSSelectorFromString(@"sharedManager")] performSelector:adidSelector] performSelector:NSSelectorFromString(@"UUIDString")];
		}
	}
	return adId;
}
...
+ (BOOL)advertisingTrackingEnabled
{
	...
	Class advertisingClass = NSClassFromString(@"ASIdentifierManager");
	if (advertisingClass) {
		id target = [advertisingClass performSelector:NSSelectorFromString(@"sharedManager")];
		if (target)
		{
			SEL trackingEnabledSelector = NSSelectorFromString(@"isAdvertisingTrackingEnabled");
			NSInvocation *trackingEnabledInvocation = [LocalyticsUtil invocationForSelector:trackingEnabledSelector target:target];
			[trackingEnabledInvocation invoke];
			[trackingEnabledInvocation getReturnValue:&enabled];
		}
	}
...
+ (NSString *)identifierForVendor
{
    ...
    {
        vendorId = (NSString *)[[device performSelector:NSSelectorFromString(@"identifierForVendor")] performSelector:NSSelectorFromString(@"UUIDString")];
    }
	return vendorId;
}
...
+ (BOOL)remoteNotificationsEnabled
{
	...
	SEL isRegisteredForRemoteNotificationsSelector = NSSelectorFromString(@"isRegisteredForRemoteNotifications");
	if ([application respondsToSelector:isRegisteredForRemoteNotificationsSelector])
	{
		NSInvocation *isRegisteredForRemoteNotificationsInvocation = [LocalyticsUtil invocationForSelector:isRegisteredForRemoteNotificationsSelector target:application];
		[isRegisteredForRemoteNotificationsInvocation invoke];
		[isRegisteredForRemoteNotificationsInvocation getReturnValue:&enabled];
	}
	else
	{
		SEL enabledRemoteNotificationTypesSelector = NSSelectorFromString(@"enabledRemoteNotificationTypes");
		NSInvocation *enabledRemoteNotificationTypesInvocation = [LocalyticsUtil invocationForSelector:enabledRemoteNotificationTypesSelector target:application];
		[enabledRemoteNotificationTypesInvocation invoke];
		
		UIRemoteNotificationType type = UIRemoteNotificationTypeNone;
		[enabledRemoteNotificationTypesInvocation getReturnValue:&type];
